...
def run_one_seed(seed: int, device: str,
    ...
    # -------- Residual-Physics NARX (delta-PINN) --------
    t0 = time.time()
    rp = ResPhysNARXHparams(window=2, hidden=(200, 400, 200), activation="tanh",
                              lr_adam=1e-3, n_epochs_adam=epochs, batch_size=64,
                              lbfgs_iters=narx_lbfgs, weight_decay=0.0,
                              early_stop_patience=120, seed=seed,
                              include_u_lags=False, device=device,
                              rk4_sub_steps=50)
    m_rp = ResPhysNARXModel(n_u=2, n_y=4, hp=rp)
    m_rp.fit(tr, va, verbose=verbose)
    out["resphys_t1_one_step"] = m_rp.eval_mae_one_step(t1)
    out["resphys_t2_one_step"] = m_rp.eval_mae_one_step(t2)
    out["resphys_t1_autoreg"]  = m_rp.eval_mae(t1)
    out["resphys_t2_autoreg"]  = m_rp.eval_mae(t2)
    out["resphys_time_s"]      = time.time() - t0
    print(f"  ResPhys  Test1 one-step = {out['resphys_t1_one_step']:.6f}  "
            f"Test2 one-step = {out['resphys_t2_one_step']:.6f}  "
            f"({out['resphys_time_s']:.0f}s)")
    return out
...
def summarize(results: list[dict]) -> str:
    keys = ["narx_t1_one_step", "narx_t2_one_step",
             ...
             "resphys_t1_one_step", "resphys_t2_one_step"]
    paper = {"narx_t1_one_step":   PAPER["narx_t1"],
              ...
              "resphys_t1_one_step": PAPER["pinarx_t1"],
              "resphys_t2_one_step": PAPER["pinarx_t2"]}
